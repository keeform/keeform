...
keeform.greenIcon = function (tabId) {
    ...
    chrome.tabs.get(tabId, function(tab) {
        ...
        chrome.browserAction.setIcon({path: 'icons/icon19o.png', tabId: tabId}, keeform.callback.bind({caller: 'greenIcon'}))
...
keeform.resetIcon = function (tabId) {
    ...
    chrome.tabs.get(tabId, function(tab) {
        if (chrome.runtime.lastError || !tab) return
        chrome.browserAction.setIcon({path: 'icons/icon19.png', tabId: tabId}, keeform.callback.bind({caller: 'resetIcon'}))
        chrome.browserAction.setBadgeBackgroundColor({color: keeform.badgeColor, tabId: tabId})
        chrome.browserAction.setBadgeText({text: ' ', tabId: tabId})
    })
}
...
keeform.showVersionWarning = function(hostVersion) {
    ...
    chrome.browserAction.setBadgeBackgroundColor({color: keeform.badgeColor})
...
// MV2: uses chrome.tabs.executeScript instead of chrome.scripting.executeScript
keeform.executeScripts = function(scripts, cb) {
    ...
    if (scripts.allFrames === undefined) {
        ...
        details = {file: file, runAt: "document_idle", frameId: scripts.frameId}
    } else {
        debug.log(padIds(scripts.tabId, scripts.frameId), 'executeScripts', file, "on tab")
        details = {file: file, runAt: "document_idle", allFrames: scripts.allFrames}
    }
    chrome.tabs.executeScript(scripts.tabId, details, function() {
...
chrome.webRequest.onAuthRequired.addListener(keeform.onAuthRequired, {urls: ['<all_urls>']}, ['blocking'])
...
port.onMessage.addListener(function(msg) {
    ...
    if (msg == 'requestSettings') {
        ...
        port.postMessage({
            ...
            protocol: 2,
            debug:    window.keeformloglevel !== undefined
        })
        return true
    }
...
// #endregion
// Expose keeform to global scope for keeform_crypto.js
window.keeform = keeform
})()
